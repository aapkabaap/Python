...
appLabel = tk.Label(root, text="Student Portal", fg="#000080", width=55)
appLabel.config(font=("Algerian", 30))
appLabel.grid(row=0, columnspan=2, padx=(10,10), pady=(30, 0))


appLabel = tk.Label(root, text="CRUD operations student portal made by Nishant", fg="#000080", width=100)
appLabel.config(font=("Algerian", 10))
appLabel.grid(row=7, columnspan=2, padx=(10,10), pady=(30, 0))
...
class Student:
    ...
    def __init__(self, studentName, schoolName, phoneNumber, address):
...
nameLabel = tk.Label(root, text="Enter your name", width=40, anchor='w',
                     font=("Arial black", 12)).grid(row=1, column=0, padx=(10,0),
                                                pady=(30, 0))
schoolLabel = tk.Label(root, text="Enter your school", width=40, anchor='w',
                        font=("Arial black", 12)).grid(row=2, column=0, padx=(10,0))
phoneLabel = tk.Label(root, text="Enter your phone number", width=40, anchor='w',
                      font=("Arial black", 12)).grid(row=3, column=0, padx=(10,0))
addressLabel = tk.Label(root, text="Enter your address", width=40, anchor='w',
                        font=("Arial black", 12)).grid(row=4, column=0, padx=(10,0))
...
def takeNameInput():
    global nameEntry, schoolEntry, phoneEntry, addressEntry
    # global username, schoolName, phone, address
    global list
    global TABLE_NAME, STUDENT_NAME, STUDENT_school, STUDENT_ADDRESS, STUDENT_PHONE
...
def destroyRootWindow():
    ...
    appLabel = tk.Label(secondWindow, text="Student Portal Database",
                        fg="#06a099", width=55)
    appLabel.config(font=("Algerian", 30))
    appLabel.pack()
...
def printDetails():
     for singleItem in list:
         print("Student name is: %s\nschool name is: %s\nPhone number is: %d\nAddress is: %s" %
               (singleItem.studentName, singleItem.schoolName, singleItem.phoneNumber, singleItem.address))
         print("****************************************")
